...
export default function MobileMoneyCheckoutModal({
  installmentId,
  amountGhs,
  purpose,
  isOpen,
  ...
  const [rail, setRail] = useState<MomoRail>('mtn_momo');
  const [phone, setPhone] = useState('');
  const [step, setStep] = useState<'details' | 'prompt' | 'success' | 'error'>('details');
  ...
  const handleInitiate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isValidGhanaMobileNumber(phone)) {
      setErrorMessage('Enter a valid 9-digit Ghana mobile number.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/escrow/payments', {
        ...
        body: JSON.stringify({ installmentId, rail, phoneNumber: phone }),
      });
  ...
  const handleConfirmPin = async () => {
    ...
    try {
      const res = await fetch(`/api/escrow/payments/${transactionId}/confirm`, { method: 'POST' });
      const data = await res.json();

      if (!res.ok) {
        setErrorMessage(data.error ?? 'Payment could not be confirmed. Please try again.');
        setStep('error');
        return;
      }
      ...
      setStep('success');
      onSuccess(data.providerReference ?? '');
    } catch {
      setErrorMessage('Network error while confirming your payment. If money left your account, contact support before retrying.');
      setStep('error');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl relative overflow-hidden">
        <button onClick={resetAndClose} className="absolute top-5 right-5 text-xl text-[#6B7E72] hover:text-[#111A14]" aria-label="Close">
          ✕
        </button>
        ...
        {step === 'details' && (
          <form onSubmit={handleInitiate} className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={n.id}
                    type="button"
                    onClick={() => setRail(n.id as MomoRail)}
            ...
            <div>
              ...
              {errorMessage && <p className="text-[11px] text-[#D94F3D] font-semibold mt-1.5">{errorMessage}</p>}
            </div>
        ...
        {step === 'prompt' && (
          <div className="text-center py-6 space-y-4">
            ...
            <p className="text-[10px] text-[#6B7E72]">
              A real deployment replaces this button with a webhook from your Mobile Money aggregator — the tenant just waits.
            </p>
          </div>
        )}
        ...
        {step === 'error' && (
          <div className="text-center py-6 space-y-4">
            <div className="w-16 h-16 rounded-full bg-[#FDECEA] text-[#D94F3D] text-3xl flex items-center justify-center mx-auto">
              ⚠️
            </div>
            <div>
              <h3 className="text-xl font-serif font-bold text-[#0F3D26]">Payment Not Completed</h3>
              <p className="text-xs text-[#6B7E72] mt-1">{errorMessage || 'Something went wrong. Please try again.'}</p>
            </div>
            <button
        ...
        {step === 'success' && (
          <div className="text-center py-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#6B7E72] mt-1">Receipt reference: <strong className="font-mono text-[#1A5C3A]">{providerReference}</strong></p>
            </div>

            <div className="p-4 rounded-2xl bg-[#EEF7F2] text-xs text-[#1A5C3A] font-medium">
              This payment is now recorded on your lease and visible to your landlord in their finances ledger.
            </div>
