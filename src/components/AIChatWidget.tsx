...
export default function AIChatWidget() {
  ...
  const [messages, setMessages] = useState([
    { role: 'bot', text: "Hello! I'm Abena, your Civitas AI Assistant. 🌿 How can I help you today with property management, solar-ready properties, or maintenance requests?" }
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Uses whichever Supabase project this deployment is actually
      // configured against, rather than a different hardcoded project URL
      // — a chat widget silently talking to a different backend than the
      // rest of the app is pointed at is its own kind of bug.
      const { data: { user } } = await supabase.auth.getUser();

      const res = await fetch(`${supabaseUrl}/functions/v1/ai-advisor`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // Omit user_id entirely when no one is signed in, rather than
        // sending a fixed shared UUID for every anonymous visitor — a
        // constant fake id would make every unauthenticated visitor look
        // like the same person to whatever's on the other end.
        body: JSON.stringify(user ? { message: userMsg, user_id: user.id } : { message: userMsg }),
      });
      const data = await res.json();
      setMessages(prev => [...prev, { role: 'bot', text: data.response || "I'm available to answer any questions about Civitas properties and maintenance requests!" }]);
    } catch {
      setMessages(prev => [...prev, { role: 'bot', text: "I'm not able to respond right now. Feel free to call our support line at +233 55 506 2589!" }]);
    } finally {
      setLoading(false);
    }
  };
