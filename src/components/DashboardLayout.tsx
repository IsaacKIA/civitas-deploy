...
const INITIAL_NOTIFICATIONS = [
  { id: '1', type: 'maintenance', title: 'Technician Dispatched', desc: 'Technician Kofi A. is en route to East Legon Villa.', time: '10m ago', unread: true },
  { id: '2', type: 'financial', title: 'Rent Payment Received', desc: 'GHS 3,200 received via MTN MoMo for Apt 3B.', time: '2h ago', unread: true },
  { id: '3', type: 'system', title: 'Solar Output Alert', desc: 'Mankessim Estate solar generation reached peak 14.2 kWh.', time: '1d ago', unread: true },
];
              ...
              <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl border border-[#D8E4DC] shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="divide-y divide-[#D8E4DC] max-h-80 overflow-y-auto">
                  {notifications.map(n => (
                    <div
                      key={n.id}
                      className={`p-4 transition-colors ${n.unread ? 'bg-[#EEF7F2]/50' : 'bg-white'}`}
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-bold text-[#111A14]">{n.title}</span>
                        <span className="text-[9px] text-[#6B7E72]">{n.time}</span>
                      </div>
